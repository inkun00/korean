...
const defaults = {
  ...
  quizAnswer: null, escapeOrder: [], transcript: "", singingScore: 0,
...
function renderHome() {
  ...
  return shell(`
    ...
    <section class="lesson-grid">${lessonCard(1,"애국가의 비밀","역사 자료를 살펴보고 네 개의 방을 탈출해요.","🗝️")}${lessonCard(2,"가사를 제대로","가사의 뜻을 익히고 무반주 노래에 도전해요.","🎙️")}${lessonCard(3,"우리의 뮤직비디오","모둠 그림을 모아 한 편의 영상을 만들어요.","🎬")}</section>
...
function escapeView() {
  const order = state.escapeOrder;
  const items = [{id:"A",label:"가사의 등장",year:"19세기 말"},{id:"B",label:"새 선율 작곡",year:"1930년대"},{id:"C",label:"광복",year:"1945년"},{id:"D",label:"오늘날의 애국가",year:"현재"}];
  const solved = order.join("") === "ABCD";
  return `<h2>🗝️ 시간의 방</h2><p class="lead">사건 카드를 오래된 순서대로 눌러 네 자리 암호를 완성하세요.</p><div class="escape-code">${order.length ? order.join(" ") : "_ _ _ _"}</div><div class="puzzle-grid">${items.map(x=>`<button class="puzzle-tile ${order.includes(x.id)?"active":""}" data-puzzle="${x.id}" ${solved?"disabled":""}><b>${x.id}</b><br>${x.label}<small style="display:block;color:#788793;margin-top:6px">${x.year}</small></button>`).join("")}</div><div class="button-row" style="margin-top:16px"><button class="btn secondary" data-action="reset-puzzle">다시 놓기</button><button class="btn coral" data-action="check-puzzle">암호 확인</button></div>${solved?`<div class="feedback ok">탈출 성공! 역사 점수 20점을 획득했어요.</div>${nextButton(1,"탐험 정리")}`:""}`;
}
...
function bindPage(){
  ...
  document.querySelectorAll("[data-puzzle]").forEach(b=>b.onclick=()=>{const id=b.dataset.puzzle;if(!state.escapeOrder.includes(id))state.escapeOrder.push(id);if(state.escapeOrder.join("")==="ABCD"){state.scores.escape=20;save()}render()});
  const resetPuzzle=$("[data-action='reset-puzzle']");if(resetPuzzle)resetPuzzle.onclick=()=>{state.escapeOrder=[];render()};
  const checkPuzzle=$("[data-action='check-puzzle']");if(checkPuzzle)checkPuzzle.onclick=()=>{if(state.escapeOrder.join("")==="ABCD"){state.scores.escape=20;save();toast("시간의 방을 탈출했어요!");render()}else{toast("순서를 다시 살펴보세요.");state.escapeOrder=[];render()}};
