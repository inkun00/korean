import { ESCAPE_DURATION_SEC, ESCAPE_ROOMS, endingFor, finalEscapeScore, newEscapeGame, nextRoom, normalizeEscapeGame, renderEscape, submitRoom, useHint } from "./escape-game.js";
...
function renderHome() {
  ...
  return shell(`
    ...
    <section class="lesson-grid">${lessonCard(1,"애국가의 비밀","핵심 기록방 10개와 보너스방 2개를 복원해요.","🗝️")}${lessonCard(2,"가사를 제대로","가사의 뜻을 익히고 무반주 노래에 도전해요.","🎙️")}${lessonCard(3,"우리의 뮤직비디오","모둠 그림을 모아 한 편의 영상을 만들어요.","🎬")}</section>
...
function currentEscapeGame(){
  state.escapeGame=normalizeEscapeGame(state.escapeGame);return state.escapeGame;
}
function escapeView() {
  return renderEscape(currentEscapeGame());
}
...
function bindPage(){
  ...
  const startEscape=$("[data-action='start-escape']");if(startEscape)startEscape.onclick=()=>{state.escapeGame={...newEscapeGame(),started:true,startedAt:Date.now()};state.scores.escape=0;save();render()};
  const resetEscape=$("[data-action='reset-escape']");if(resetEscape)resetEscape.onclick=()=>{state.escapeGame=newEscapeGame();state.scores.escape=0;save();render()};
  document.querySelectorAll("[data-escape-token]").forEach(b=>b.onclick=()=>{const game=currentEscapeGame(),id=b.dataset.escapeToken;if(!game.interaction.sequence.includes(id))game.interaction.sequence.push(id);game.feedback=null;save();render()});
  document.querySelectorAll("[data-escape-toggle]").forEach(b=>b.onclick=()=>toggleEscapeSelection(b.dataset.escapeToggle));
  document.querySelectorAll("[data-escape-choice]").forEach(b=>b.onclick=()=>{const game=currentEscapeGame();game.interaction.selected=[b.dataset.escapeChoice];game.feedback=null;save();render()});
  document.querySelectorAll("[data-drag-item]").forEach(b=>{b.ondragstart=e=>e.dataTransfer.setData("text/plain",b.dataset.dragItem);b.onclick=()=>{const game=currentEscapeGame(),id=b.dataset.dragItem;game.interaction.held=id;game.interaction.openedItem=id;game.feedback=null;save();render()}});
  document.querySelectorAll("[data-drop-zone]").forEach(zone=>{zone.ondragover=e=>e.preventDefault();zone.ondrop=e=>{e.preventDefault();placeEscapeItem(e.dataTransfer.getData("text/plain"),zone.dataset.dropZone)};zone.onclick=()=>{const game=currentEscapeGame();if(game.interaction.held)placeEscapeItem(game.interaction.held,zone.dataset.dropZone)}});
  const clearEscape=$("[data-action='clear-escape-selection']");if(clearEscape)clearEscape.onclick=()=>{const game=currentEscapeGame();game.interaction.sequence=[];game.feedback=null;save();render()};
  const submitEscape=$("[data-action='submit-escape-room']");if(submitEscape)submitEscape.onclick=()=>{submitRoom(currentEscapeGame());save();render()};
  const escapeHint=$("[data-action='escape-hint']");if(escapeHint)escapeHint.onclick=()=>{useHint(currentEscapeGame());save();render()};
  const nextEscape=$("[data-action='next-escape-room']");if(nextEscape)nextEscape.onclick=()=>{const game=currentEscapeGame();nextRoom(game);if(game.roomIndex>=ESCAPE_ROOMS.length)state.scores.escape=finalEscapeScore(game);save();render()};
  const finishEscape=$("[data-action='finish-escape']");if(finishEscape)finishEscape.onclick=()=>{state.scores.escape=finalEscapeScore(currentEscapeGame());state.lessonStep[1]=3;save();render()};
...
function toggleEscapeSelection(id){const game=currentEscapeGame(),room=ESCAPE_ROOMS[game.roomIndex];if(room.mechanic==="choice")game.interaction.selected=[id];else game.interaction.selected=game.interaction.selected.includes(id)?game.interaction.selected.filter(x=>x!==id):[...game.interaction.selected,id];game.feedback=null;save();render()}
function placeEscapeItem(item,zone){if(!item)return;const game=currentEscapeGame();game.interaction.placements[item]=zone;game.interaction.held=null;game.feedback=null;save();render()}
function startEscapeClock(){
  const update=()=>{const el=$("#escape-time");if(!el)return;const game=currentEscapeGame();const left=Math.max(0,ESCAPE_DURATION_SEC-Math.floor((Date.now()-game.startedAt)/1000));el.textContent=`${String(Math.floor(left/60)).padStart(2,"0")}:${String(left%60).padStart(2,"0")}`;if(left===0&&!game.forcedEnding){game.forcedEnding="bad_ending";save();render()}};update();escapeTimer=setInterval(update,1000);
}
